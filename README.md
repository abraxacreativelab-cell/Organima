...
## Ejecutar
Node.js 22 o superior. `npm ci`, copiar `.env.example` a `.env`, `npm run dev`. Abrir http://localhost:3210. `npm test` y `npm run check` verifican la base digital. La simulación está identificada y no necesita credenciales. El modo live requiere Tavily y modelos NVIDIA disponibles en Nebius.
